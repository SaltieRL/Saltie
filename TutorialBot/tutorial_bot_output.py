...
class TutorialBotOutput:
    ...
    def aim(self, target_x, target_y):
        angle_between_bot_and_target = self.to_degrees(tf.atan2(tf.subtract(target_y, bot_pos.Y), tf.subtract(target_x, bot_pos.X)))
        ...
        ps = tf.cond(tf.less(tf.abs(self.to_degrees(angle_front_to_target)), powerslide_angle), lambda: True,
                     lambda: False)
        return [st, ps]
